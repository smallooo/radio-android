...
@HiltViewModel
class FavoriteStationstViewModel @Inject constructor(savedStateHandle: SavedStateHandle, val stationsRepository: StationsRepository, val preferences: PreferencesStore) : ViewModel() {
    ...
    val favoriteStationsState1: StateFlow<StationsUiState> = combine(
        stationsRepository.getStationbyIdsEntitiesStream(
            listOf<String>(
    ...
    fun getFavoritedStation(stationUUID: ArrayList<String>) =
        viewModelScope.launch {
            stationsRepository.getStationbyIdsEntitiesStream(stationUUID).stateIn(
                scope = viewModelScope,
                started = SharingStarted.WhileSubscribed(5_000),
                initialValue = StationsUiState.Loading
            )
     ...
     fun getFavoritedStation1() {
         ...
         GlobalScope.launch (Dispatchers.IO) {
          ...
          stationsRepository.getStationbyIdsEntitiesStream(aaa)
     }
